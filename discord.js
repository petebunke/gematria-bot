 ...
 */
function formatGematriaMessage(gematriaData) {
    ...
    try {
        ...
        if (options.embed) {
            const embed = {
                ...
                footer: {
                    text: options.embed.footer || 'Aik Bekar⁹'
                }
            };

            // Only add title if explicitly provided
            if (options.embed.title) {
                embed.title = options.embed.title;
            }
        ...
        if (options.filePath && fs.existsSync(options.filePath)) {
            const { FormData, File, fetch: undiciFetch } = await import('undici');
